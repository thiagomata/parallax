import{D as L,S as M,a as T,P as N,W as q,b as R,C as F,c as I,E as c,d as z}from"./colors-DVKk-YNe.js";import{t as D,P as m,s as k,e as W}from"./tutorial.template-CuDnZY33.js";const H=`
<div class="concept">
...
`;async function G(s,a,y,d,p="",o={}){const u=document.getElementById("tutorial-root");if(!u)return;let r=document.getElementById(`step-${s}`);if(!r)r=D({containerId:s,title:a,source:d,explanation:p}),u.appendChild(r);else{const e=r.querySelector('[data-content="preview"]'),t=r.querySelector('[data-content="code"]'),n=r.querySelector('[data-content="learn"]');if(e){const l=e.querySelector(".step-anchor");l&&(l.innerHTML=`<span>${a}</span>`)}if(t){const l=t.querySelector(".step-anchor");l&&(l.innerHTML=`<div class="status-dot"></div><span>${a}</span>`);const C=t.querySelector(".editor-box code");C&&(C.innerHTML=m.highlight(d,m.languages.typescript,"typescript"))}if(n){const l=n.querySelector(".learn-content");l&&(l.innerHTML=p)}}const b=r.querySelector(".editor-box"),g=r.querySelector(".canvas-box"),E=r.querySelector(".console-panel"),x=r.querySelector(".play-btn"),w=r.querySelector(".reset-btn"),v=r.querySelector(".copy-btn"),i=r.querySelector(".pause-btn"),h={...L,...o,width:g?.clientWidth||500,height:g?.clientHeight||400,paused:!1};let{currentP5:f}=await k.createSketchInstance(y,h,g,s);const S=r.querySelectorAll(".tab-btn"),A=r.querySelectorAll(".step-content");S.forEach(e=>{e.addEventListener("click",()=>{const t=e.getAttribute("data-tab");S.forEach(n=>n.classList.remove("active")),e.classList.add("active"),A.forEach(n=>{n.getAttribute("data-content")===t?n.classList.add("active"):n.classList.remove("active")})})});const P=async()=>{E.style.display="block",E.innerHTML="";try{const{fn:e}=W(b.innerText);f=(await k.createSketchInstance(e,h,g,s)).currentP5,h.paused=!1,i.dataset.paused="false",i.innerHTML='<i class="fas fa-pause"></i>';const n=document.createElement("div");n.className="log-entry info",n.textContent="[Engine] Hydration successful.",E.appendChild(n)}catch(e){console.error("executeUpdate error:",e);const t=document.createElement("div");t.className="log-entry info",t.style.color="var(--error)",t.textContent=e instanceof Error?`Error: ${e.message}

Stack:
${e.stack}`:String(e),t.style.whiteSpace="pre-wrap",t.style.fontSize="11px",E.appendChild(t)}};x.addEventListener("click",P),w.addEventListener("click",()=>{const e=r.querySelector(".editor-box code");e&&(e.innerHTML=m.highlight(d,m.languages.typescript,"typescript")),P()}),v.addEventListener("click",()=>navigator.clipboard.writeText(b.innerText));const _=()=>/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)||navigator.maxTouchPoints>0&&window.innerWidth<769;r.querySelectorAll(".fullscreen-btn").forEach(e=>{if(_()){e.style.display="none";return}e.addEventListener("click",()=>{const t=r.querySelector(".canvas-box");t&&(document.fullscreenElement||document.webkitFullscreenElement?document.exitFullscreen?.()||document.webkitExitFullscreen?.():(t.requestFullscreen?.bind(t)||t.webkitRequestFullscreen?.bind(t))?.())})}),i.addEventListener("click",()=>{if(!f)return;i.dataset.paused==="true"?(f.loop(),h.paused=!1,i.dataset.paused="false",i.innerHTML='<i class="fas fa-pause"></i>'):(f.noLoop(),h.paused=!0,i.dataset.paused="true",i.innerHTML='<i class="fas fa-play"></i>')})}G("hero-demo-1","Hero Demo",B,O,H);
//# sourceMappingURL=main-DfbvjXSs.js.map
