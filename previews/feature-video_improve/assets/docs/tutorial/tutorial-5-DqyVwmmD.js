import{D as p,S as h,a as i,c as d,P as u,W as g,b as m,E as l}from"../../tutorial.template-DfAWeqR7.js";/* empty css                     */import{i as y,a as w}from"../../tutorial_shared-CoO0eFCZ.js";y();const T=`
<div class="concept">
...
`;async function f(o,t=p,n){let a;const c=t.clock??new h({...i,startPaused:t.paused,playback:{...i.playback,duration:5e3,isLoop:!0}}),s=new u(o);a=n?.graphicProcessor??new d(o,s);const e=new g(m.fromLibs({clock:c,loader:s}));return e.startLoading(),e.enableDefaultPerspective(t.width,t.height),t.paused&&e.pause(),o.setup=async()=>{o.createCanvas(t.width,t.height,o.WEBGL),n?.graphicProcessor||(a=new d(o,s)),e.addBox({type:l.BOX,id:"textured-box",width:150,position:{x:0,y:0,z:-100},strokeWidth:0,rotate:r=>({pitch:0,yaw:Math.PI*2*r.playback.progress,roll:Math.PI*2*r.playback.progress}),texture:{path:"/parallax/img/target.png",width:100,height:100}}),e.addText({type:l.TEXT,id:"title",text:"TEXTURES",size:30,position:{x:0,y:0,z:50},font:{name:"Roboto",path:"/parallax/fonts/Roboto-Regular.ttf"},fillColor:{red:0,green:229,blue:255}}),await s.waitForAllAssets()},e.complete(),o.draw=async()=>{t.paused&&!e.isPaused()?e.pause():!t.paused&&e.isPaused()&&e.resume(),o.background(15),(await e.step(a)).running},e}const x=`import p5 from 'p5';
...
`;w("tutorial-5","5. Loading Textures",f,x,T);
//# sourceMappingURL=tutorial-5-DqyVwmmD.js.map
